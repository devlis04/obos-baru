...
class AbsensiRepo {
  ...
  Future<int?> idBukuTerbuka() async {
    final hasil = await _sb.rpc('setoran_buku_terbuka').timeout(Jaringan.lambat);
    if (hasil == null) return null;
    if (hasil is int) return hasil;
    if (hasil is num) return hasil.toInt();
    return int.tryParse(hasil.toString());
  }
  ...
  Future<StatusAbsensi> status() async {
    final hasil = await _sb.rpc('gudang_status_lantai').timeout(Jaringan.lambat);
