...
class ScanLayar extends StatefulWidget {
  const ScanLayar({
    super.key,
    required this.toko,
    this.keluar = false,
    this.tampilBypass = true,
  });

  final Pelanggan toko;
  final bool keluar;
  final bool tampilBypass;

  @override
  State<ScanLayar> createState() => _ScanLayarState();
}

class _ScanLayarState extends State<ScanLayar> {
  ...
  Future<void> _hidupkanGps() async {
    try {
      ...
      _gps = Geolocator.getPositionStream(
        ...
        onError: (_) {
          if (mounted) setState(() => _status = 'GPS terganggu. Tunggu sebentar…');
        },
      );
    } catch (_) {
  ...
  void _onKode(String kode) {
    ...
    if (kode.trim() != widget.toko.id.trim()) {
      _kodeTahan = null;
      _pesan('QR ini bukan milik toko yang sedang dikunjungi. Scan QR toko yang benar.');
      return;
    }
    final pos = _pos;
    ...
    if (d > _batasMeter) {
      _pesan(
        'Dekati dulu (${d.toStringAsFixed(0)} m).',
      );
      return;
    }
  ...
  Future<void> _kirim(Position pos) async {
    ...
    try {
      ...
      final waktu = await _repo.scanKunjungan(
        id: widget.toko.id,
      ...
      if (widget.keluar) {
        final catat = CatatKunjungan(
          id: widget.toko.id,
          waktuKeluar: waktu,
        );
        context.read<PelangganBloc>().add(catat);
        await catat.selesai.future;
        if (!mounted) return;
      ...
      final toko = widget.toko.salin(waktuMasuk: waktu);
      final catat = CatatKunjungan(id: toko.id, waktuMasuk: waktu);
      context.read<PelangganBloc>().add(catat);
      await catat.selesai.future;
      if (!mounted) return;
  ...
  String _pesanGagal(Object error) {
    ...
    return 'Terjadi gangguan saat memproses kunjungan. Tunggu sebentar, lalu coba lagi.';
  }

  Future<void> _bypass() async {
    if (_busy || widget.keluar) return;
    await KunjunganSesi.setSesi(
      toko: widget.toko,
      bypass: true,
      hari: await KunjunganSesi.hariDaftar(context),
      ...
      body: !_netSiap
          ...
          : Stack(
              children: [
                ...
                if (widget.tampilBypass && !widget.keluar && !_busy)
                  Positioned(
                    bottom: 32,
                    left: 24,
                    right: 24,
