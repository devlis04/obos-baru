-- Chip supplier kartu barang masuk: ikut buku terbuka, termasuk baris
-- yatim (belum cap) tanggal buku atau hari ini (Jakarta).
-- 055 menimpa ringkas 066; jalankan SETELAH 077. Boleh diulang.
...
AS $$
  SELECT
    ...
    OR (
      ...
      AND p_tgl_baris IS NOT NULL
      AND (
        p_tgl_baris = p_tgl
        OR p_tgl_baris = (timezone('Asia/Jakarta', clock_timestamp()))::date
      )
    );
$$;
